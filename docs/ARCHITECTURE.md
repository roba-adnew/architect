...
- **Mechanism:**
  ...
  - Identity is per session: the session is named `architect-<persist_index>`, a per-session identity assigned at creation and never rewritten by grid compaction, and the socket is a single stable per-user path, so reattach finds it across restarts. (Keying the name off the mutable grid `slot_index` used to let a new terminal attach to — and clone — a live session; see the slot-name limitation below.)
...
- **Known limitations (accepted; phase-1 scope):**
  ...
  - *Single instance* — the socket and per-session names are shared per user; two Architect instances would collide on `architect-<n>`.
  - *Slot-name stability* — the session name now follows a per-session `persist_index` (stable within a run), which fixed the bug where a new terminal would attach to and clone a live session as the grid reordered. Across a full restart the reattach still maps by physical slot, so a persisted per-conversation token is still needed to guarantee the right agent lands in the right slot.
