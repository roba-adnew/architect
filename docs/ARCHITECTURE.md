...
- **Known limitations (accepted; phase-1 scope):**
  ...
  - *Scrollback under tmux* — live history lives in tmux's 50k-line buffer, not Architect's. Wheel-scroll reaches it via tmux copy-mode (`tmux.scrollHistory`); the cost is that while scrolled up the pane sits in copy-mode, so keystrokes are captured until you scroll back to the bottom (`-e` auto-exits). On reattach, history above the visible screen is still not repainted Architect-side. Conversation state is intact.
