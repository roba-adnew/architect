...
- **Live session persistence (experimental)** — when `tmux` is installed, each shell runs inside a detached tmux session by default, so agents *survive* an Architect restart and reattach with full in-memory context instead of being resumed from a transcript. See [Configuration](docs/configuration.md#persistent-agent-sessions) and ADR-015 for details and current limitations.
...
- **Grid view** — keep all agents visible simultaneously; single-click a pane to move focus to it (no shell is spawned), double-click to expand it to full screen; double-click a focused pane to return to the grid (works even over mouse-tracking programs like Claude/vim)
- **Drag to reorder** — press and hold a grid tile (~¼s without moving), then drag: as the cursor reaches the center of another tile, tiles slide springboard-style to make room (only the tiles between the old and new slot shift). Release to drop, Esc to snap the tile back to where it started. The arrangement survives closing terminals and app restarts; a revealed hidden terminal joins at the end of the grid rather than disturbing your arrangement. Moving the pointer right away instead selects text as usual, and Cmd+Click still opens links
- **Find in terminal** (⌘F) — search the focused pane's scrollback and visible screen (case-insensitive substring); the current match is highlighted and the view scrolls to it. Enter/Shift+Enter (or ↑/↓) jump between matches, the counter shows position (e.g. `3/12`), and Esc clears the search and returns to the live prompt
...
- **Hide terminals** (⌘J / ⌘⇧J) — pull a terminal out of the grid with ⌘J to declutter (it keeps running in the background); press ⌘⇧J to open a searchable switcher listing every hidden terminal as `agent.folder` (just the folder when no agent is running) with live agent status (needs you / working / done / idle — driven by Claude's hooks: "needs you" only appears for real permission/input requests (not a session idling at the prompt), "working" only shows while Claude is actually processing, and "needs you" / "done" are sticky cues that clear back to idle once you view the terminal; "hosting" when the terminal is serving a local TCP port, and a plain dash when Claude never ran there), then filter by typing the folder or agent name, navigate with arrows or the mouse, and Enter/click to bring one back into the grid. A count pill in the top-right corner shows how many are hidden, and hidden terminals stay hidden across restarts
...
- Keyboard navigation: ⌘+Return to expand, ⌘1–⌘0 to switch grid slots, ⇧Arrow to move focus in grid view (plays a brief wave animation on the destination terminal; ⌘←/→ instead jump to line start/end in the focused terminal), ⌘N to add, ⌘W to close a terminal (restarts if it's the only terminal), ⌘J to hide a terminal from the grid (⌘⇧J opens the searchable hidden-terminals picker), ⌘T for worktrees, ⌘O for recent folders, ⌘D for repo-wide git diff (staged + unstaged + untracked), ⌘R for reader mode, ⌘F to find in the focused terminal, ⌘⇧↓ to jump the focused terminal to the bottom of its scroll (sends Ctrl+End — useful in Claude Code, where it's normally Ctrl+End but macOS window managers grab the physical Ctrl+Fn+→), ⌘/ for shortcuts; in kitty-protocol apps (e.g. Claude Code) modified Tab/Enter/Backspace are sent as real key events, so ⌥Enter inserts a newline and ⌥Delete deletes a word per the app's own bindings; quit with ⌘Q or the window close button (if terminals are still running, a 4-second **Undo Close** window lets you cancel before it quits)
...
- OSC 8 hyperlink support (Cmd+hover to underline, Cmd+Click to open); works in both full view and grid view, including detected URLs and existing file paths. Relative OSC 8 targets (e.g. `docs/FOO.md`, as emitted by Claude Code for local files) are resolved against the pane's working directory. Shells get `FORCE_HYPERLINK=1` so CLIs that feature-detect hyperlink support (Claude Code and friends) emit real OSC 8 links instead of plain colored text (export `FORCE_HYPERLINK=0` to opt out)
...
Common settings include font family, theme colors, grid font scale, and logging minimum severity (`[logging].min_level`). Set `[theme].mode = "light"` for a light theme matched to the Cacha companion app (background `#D6D6D5` + a readability-tuned light palette); see [`docs/configuration.md`](docs/configuration.md#light-mode). On macOS, structured app logs are written to `~/Library/Logs/Architect/` with size-based rotation at 10 MiB, including startup/shutdown markers and grid/full view transition events at `INFO`. The grid size is dynamic and adapts to the number of terminals. Remove the files to reset to the default values.
