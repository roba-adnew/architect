...
- **Hide terminals** (⌘J / ⌘⇧J) — pull a terminal out of the grid with ⌘J to declutter (it keeps running in the background); press ⌘⇧J to open a searchable switcher listing every hidden terminal as `agent.folder` (just the folder when no agent is running) with live agent status (needs you / working / done / idle), then filter by typing the folder or agent name, navigate with arrows, and Enter to bring one back into the grid
