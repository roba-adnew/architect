...
- Optional persistent agent sessions (`ARCHITECT_PERSIST_SESSIONS=1`, requires `tmux`) wrap each shell in a detached tmux session so agents survive an Architect restart and reattach live; logic lives in `src/tmux.zig`, wired through `src/shell.zig` (`Shell.spawn`) and `src/session/state.zig`. Unset = direct shell, unchanged. See ADR-015. Wheel-scroll is routed to tmux copy-mode (`tmux.scrollHistory`) since tmux owns the scrollback; the next keystroke cancels copy-mode (`tmux.cancelCopyMode` from `sendInput`) so the cursor returns to the prompt. Known phase-1 gaps: attention border not restored on reattach; reboot falls back to fresh/`--resume`; single-instance only.
