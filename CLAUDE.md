...
- Grid display order = `SessionState.spawn_seq` (monotonic creation counter, kept across in-place restarts, cleared on close), enforced by `compactSessions` in `src/app/runtime.zig`. Never order the grid by `id`/`persist_index` — those recycle a closed terminal's struct index, so sorting by them scrambles the visible order after close+⌘N.
