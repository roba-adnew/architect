...
- Persistent agent sessions are on by default (no opt-in): whenever `tmux` is on `PATH`, each shell is wrapped in a detached tmux session so agents survive an Architect restart and reattach live; logic lives in `src/tmux.zig`, wired through `src/shell.zig` (`Shell.spawn`) and `src/session/state.zig`. No tmux on `PATH` = direct shell, unchanged. See ADR-015. tmux session names are `architect-<epoch>-<persist_index>`: `persist_index` is the stable per-session id (NOT the mutable grid `slot_index`) so a new terminal can't attach to/clone a live session WITHIN a run; `<epoch>` is a random per-install token (`config.Persistence.persist_epoch`, minted first launch, set via `tmux.setEpoch` before any spawn) so a new run can't reattach to a same-named fossil left on the shared socket ACROSS runs. `persist_index` is persisted per terminal (`terminal_persist_indices`) and reclaimed on restore via `claimPersistIndex` (swap keeps indices unique), so a restored terminal reattaches to ITS OWN session — restoring by grid position orphaned drift-indexed terminals and the reaper then killed them. `tmux.reapDetachedSessions` runs once at startup after reattach to drain the detached-session graveyard: attached sessions are spared, persisted (`keep`) indices are spared with a warn (missed reattach), other-epoch names are always reaped, and same-epoch names are reaped only when every restored entry carried an index (legacy files skip same-epoch reaping for one launch). Duplicate persisted resume ids are resumed once (first entry wins) to avoid forking a conversation into two terminals. Closing a terminal KILLS its tmux session (`tmux.killSession` from `despawn`; quit detaches only), and every spawn wrapper except the restore path's direct `ensureSpawnedWithDir` is FRESH (discards orphans first) — otherwise a spawn reusing a closed terminal's persist_index would `new-session -A` onto the leftover and resurrect it as an exact clone. Resume-id note: a restored `agent_session_id` is seeded with `agent_metadata_captured = true` so periodic saves keep persisting it forward instead of wiping it (it's overwritten by the live SessionStart hook / quit scrape when a fresher id arrives). The live SessionStart hook is what puts the id on disk BEFORE a reboot (a reboot SIGKILLs the process with no shutdown scrape), so Architect auto-installs Claude's hooks at startup via `shell.ensureClaudeHookInstalled` (runs the idempotent `architect hook claude`, which now starts from an empty settings file when none exists) before the restore spawns — resume-across-reboot no longer depends on the user having run `architect hook claude` by hand. When a SessionStart hook captures a fresh id mid-frame, the loop re-syncs terminal entries and flushes the same frame (not one frame later) to shrink the reboot-loss window. Codex/Gemini still lack a live-capture hook (id only scraped at graceful quit), so their conversations do not survive a reboot yet. `ARCHITECT_RESUME_CMD` is consume-once on the struct (cleared after spawn) and ALWAYS overridden per tmux session (`-e`, empty when no resume) — the tmux server globals capture the starting client's env, and an un-overridden global made every new terminal run a stale `claude --resume` (`tmux.scrubStaleGlobalEnv` at startup heals servers poisoned by older builds). Wheel-scroll is routed to tmux copy-mode (`tmux.scrollHistory`) since tmux owns the scrollback; the next keystroke cancels copy-mode (`tmux.cancelCopyMode` from `sendInput`) so the cursor returns to the prompt. Known phase-1 gaps: attention border not restored on reattach; reboot falls back to fresh/`--resume` (now reliable for Claude on macOS via the auto-installed live SessionStart id-capture hook; still bare-shell for Codex/Gemini and off-macOS); single-instance only (per-`ARCHITECT_CONFIG_DIR` epochs are disjoint, but instances sharing one `persistence.toml` still share an epoch).
...
- Grid display order = `SessionState.order_seq` (formerly `spawn_seq`: a monotonic counter assigned at first spawn, renumbered by drag-reorder via `session_state.mintOrderSeqBlock`, kept across in-place restarts, cleared on close), enforced by `compactSessions` in `src/app/runtime.zig`. `applyReorder` commits a user reorder by renumbering `order_seq` and delegating the physical permutation to `compactSessions` — never permute `sessions`/`views`/`render_cache` directly, and never order the grid by `id`/`persist_index` — those recycle a closed terminal's struct index, so sorting by them scrambles the visible order after close+⌘N.
